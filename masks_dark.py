...
def get_dark_mask(data: np.ndarray,sza, doy) -> bool:
    ...
    rdn2 = ToaRef(bandrange[1],data, sza, doy)
    is_dark1 = rdn1 < threshold[0] 
    is_dark2 = rdn2 < threshold[1] 
    is_dark = np.logical_and(is_dark1[:,:,0] == 1, is_dark2[:,:,0] == 1)    
    return is_dark
...
for f in range(0,len(files)): #Go through each row of text file
    ...
    doy = GetDOY(linename)
    sat_mask_full2 = get_dark_mask(rdn_file, sza, doy) # For cloud mask
    #Specify output type
    ...
    output_filename_rgb = f_txt[:len('xxxYYYYMMDDtHHMMSS')]  + '_dark_rgb' + f_txt[len('xxxYYYYMMDDtHHMMSS_rdn_'):len('xxxYYYYMMDDtHHMMSS_rdn_v2x1')] + '_' + f_txt[len('xxxYYYYMMDDtHHMMSS_rdn_v2x1_'):len('xxxYYYYMMDDtHHMMSS_rdn_v2x1_clip')] 
