...
for f in range(0,len(files)): #Go through each row of text file
    ...
    num_img_tiles = round(rdn_file.shape[0]//size)# number of tiles to make 
    print('number of tiles for this image is: ',num_img_tiles)
    
    
    for i in range(0,num_img_tiles):
        start = i * size
        RGBplus = rdn_file[0:size,0:size,0:3] # create the output file
        #print(data.shape)
        RGBplus[:,:,0:1]= percent_stretch(rdn_file[0+start:size+start,0:size,59])#R
        RGBplus[:,:,1:2]= percent_stretch(rdn_file[0+start:size+start,0:size,38])#G
        RGBplus[:,:,2:3]= percent_stretch(rdn_file[0+start:size+start,0:size,16])#B
        
        cloud = rdn_file[0:size,0:size,0:3] # create the output file
        #print(data.shape)
        cloud[:,:,0]= sat_mask_full2[0+start:size+start,0:size]
        cloud[:,:,1]= sat_mask_full2[0+start:size+start,0:size]
        cloud[:,:,2]= sat_mask_full2[0+start:size+start,0:size]
       
        
        path_exists = os.path.exists(in_path+f_txt+ '_'+str(i+1))
        if path_exists == True:
            matplotlib.image.imsave(f_txt+ '_'+str(i+1)+'/RGB.png', RGBplus[:,:,0:3])
            matplotlib.image.imsave(f_txt+ '_'+str(i+1)+'/cloud.png',  cloud[:,:])
        else:
            os.mkdir(in_path+f_txt+ '_'+str(i+1))
            matplotlib.image.imsave(f_txt+ '_'+str(i+1)+'/RGB.png', RGBplus[:,:,0:3])
            matplotlib.image.imsave(f_txt+ '_'+str(i+1)+'/cloud.png',  cloud[:,:])
                                                                            
        print('output saved', f_txt+ '_'+str(i+1))
